...
基于 **BOSS 直聘真实招聘数据**，按你的实际底子匹配 AI 岗位，生成定制学习计划。
...
| 命令 | 触发方式 | 作用 |
|------|----------|------|
| `/job-init` | `"初始化画像"` 或首次自动触发 | 采集画像，判定 S0-S3 档位 |
| `/job-scan` | `"帮我找岗位"` | 推荐岗位 → CDP 抓取 → 清洗导出 |
| `/job-analyze` | `"分析这些岗位"` | JD 分析 → 差距分析 → 学习计划 |
...
| 模块找不到 | 在项目根目录执行 `pip install -r requirements.txt` |
| 岗位匹配少 | 尝试不同关键词或调整目标城市 |
| 数据隐私 | 所有数据存在本地，不会被上传 |

> ⚠️ **利益中立声明**：本工具不会提供"AI 高薪岗位 Top10"这类水文清单——那些多是卖课/培训机构的获客漏斗。本工具基于 BOSS 直聘真实招聘数据，匹配你的实际底子，不做速成承诺。
...
<a href="https://star-history.com/#qxiansheng001/ai-job-hunter&Date">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=qxiansheng001/ai-job-hunter&type=Date&theme=dark" />
    <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=qxiansheng001/ai-job-hunter&type=Date" />
    <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=qxiansheng001/ai-job-hunter&type=Date" />
  </picture>
</a>
